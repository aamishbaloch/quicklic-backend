...
def cancel_appointments_of_day_and_send_notify(date_to_cancel):
    date_start = get_start_datetime_from_date_string(str(date_to_cancel.date()))
    date_end = get_end_datetime_from_date_string(str(date_to_cancel.date()))

    appointments = Appointment.objects.filter(start_datetime__gte=date_start, end_datetime__lte=date_end)
